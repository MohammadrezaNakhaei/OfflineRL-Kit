...
from offlinerlkit.utils.modify_env import SimpleModifiedENV, SemiSimpleModifiedENV, ModifiedENV
...
def train(mainargs=get_args()):
    ...
    eval_env = ModifiedENV(eval_env, mainargs.max_delta)
    # seed
    ...
    res_actor = Actor(res_actor_backbone, args.action_dim, device=args.device)
    ...
    # log
    log_dirs = make_log_dirs(f'{args.task}-adaptive', f'{args.algo_name}_td3', mainargs.seed, vars(mainargs), record_params=['tag'])
    ...
    reward_offline, std_offline = res_agent.evaluate(res_agent=False)
